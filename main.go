...
func (app *myApp) CreateAndRunAppService(serviceKey string, newServiceFactory func(string) (interfaces.ApplicationService, bool)) int {
	...
	err = app.service.SetDefaultFunctionsPipeline(
		...
		// Response data returns JSON (needed by eKuiper and other standard functions)
		sample.SetResponseData)
		// Convert and ourput xml transform and return XML to bus (it may be understood only by custom apps)
	...
	err = app.service.AddFunctionsPipelineForTopics("Int32s", []string{"events/device/device-virtual/+/+/Int32"},
		...
		// Response data returns JSON
		sample.SetResponseData)
		// Lines to return XML instead of JSON
		// sample.ConvertEventToXML,
		// sample.OutputXML)
